...
export const uiKitService = {
    ...
    getMy: () =>
        apiFetch("/api/ui-kits/seller/my"),
    ...
    updateStatus: (id, status) =>
        apiFetch(`/api/ui-kits/${id}/status`, { method: "PATCH", body: JSON.stringify({ status }) }),
};
...
export const liveClassService = {
    getAll: (params = "") => apiFetch(`/api/live-classes${params}`),

    getById: (id) => apiFetch(`/api/live-classes/${id}`),

    join: (id) => apiFetch(`/api/live-classes/${id}/join`, { method: "POST" }),

    leave: (id) => apiFetch(`/api/live-classes/${id}/leave`, { method: "POST" }),

    getSignature: (id) => apiFetch(`/api/live-classes/${id}/signature`),

    getMyUpcoming: () => apiFetch('/api/live-classes/my-upcoming'),

    create: (data) => apiFetch("/api/live-classes", { method: "POST", body: JSON.stringify(data) }),

    update: (id, data) => apiFetch(`/api/live-classes/${id}`, { method: "PATCH", body: JSON.stringify(data) }),

    delete: (id) => apiFetch(`/api/live-classes/${id}`, { method: "DELETE" }),

    getMyClasses: () => apiFetch('/api/live-classes/instructor/my-classes'),
};
...
export const blogService = {
    ...
    getFeatured: () =>
        apiFetch("/api/blogs/featured"),

    getPopular: () =>
        apiFetch("/api/blogs/popular"),

    getRecent: () =>
        apiFetch("/api/blogs/recent"),

    getById: (id) =>
        apiFetch(`/api/blogs/${id}`),

    getBySlug: (slug) =>
        apiFetch(`/api/blogs/slug/${slug}`),
};
// ==================== SPREADSHEET SERVICE ====================
