...
import {
    ...
    FiCheck, FiClock, FiX, FiArrowRight, FiFileText, FiTag, FiShoppingBag, FiRefreshCw
} from "react-icons/fi";
...
export default function UserOrdersPage() {
    ...
    const getStatusStyle = (status) => {
        switch (status) {
            ...
            case "approved":
                return "bg-blue-50 dark:bg-blue-900/20 text-blue-600 border-blue-100 dark:border-blue-800/30";
            case "pending":
                                ...
                                <div className="flex flex-col md:flex-row md:items-center gap-6">
                                    ...
                                    <div className="flex md:flex-col items-center md:items-end justify-between md:justify-center gap-2 pr-4 self-stretch border-t md:border-t-0 md:border-l border-gray-50 dark:border-gray-700/50 pt-4 md:pt-0 md:pl-8">
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            {(order.paymentStatus === "completed" || order.paymentStatus === "approved") && (
                                                <button
                    ...
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                        ...
                        >
                            ...
                            <div className="p-8 bg-gray-50 dark:bg-gray-900/50 flex gap-4">
                                {(selectedOrder.paymentStatus === 'completed' || selectedOrder.paymentStatus === 'approved') ? (
                                    <Link
                                        href={selectedOrder.items?.[0]?.productType === 'course' ? `/dashboard/user/courses` : `/dashboard/user/downloads`}
                                    ...
                                    >
                                        {selectedOrder.items?.[0]?.productType === 'course' ? 'Go to My Courses' : 'Go to Downloads'} <FiArrowRight />
                                    </Link>
                                ) : selectedOrder.paymentStatus === 'pending' ? (
                                    <div className="flex-1 py-4 bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 font-black uppercase text-xs tracking-widest rounded-2xl flex items-center justify-center gap-2">
                                        <FiClock /> Waiting for Admin Approval
                                    </div>
                                ) : (
                                    <button className="flex-1 py-4 bg-red-500 text-white font-black uppercase text-xs tracking-widest rounded-2xl flex items-center justify-center gap-2">
